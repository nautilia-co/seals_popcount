...
random.seed(seed)
dataset_size = 18000  # Including images of both types (seals & no_seals)
percentage_without_seals = 0.6  # Percentage of images of class no seals. Set to 0 = all images have seals
augment_dataset = False  # Whether to apply data augmentation
...
generator_train = ExtractsGenerator(dataset=partitions['train'], batch_size=batch_size, x_shape=input_shape,
                                    y_size=output_nodes, data_augmentation=augment_dataset)
...
# Create ResNet
# resnet = ResNet(final_activation='softmax', n=n,
